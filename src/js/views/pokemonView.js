...
export const renderPokemon = (pokeData, isLiked) => {
    document.querySelector(".primaryInfo__left").innerHTML = renderPrimaryLeft(pokeData.primary);
    document.querySelector(".primaryInfo__right").innerHTML = renderPrimaryRight(pokeData.primary, pokeData.flavorText, pokeData.ev, pokeData.eggData, pokeData.genders, pokeData.catchRate);
    document.querySelector(".evolution").innerHTML = renderEvolutionTree(pokeData.evolutionTree);
    document.querySelector(".extra-stats").innerHTML = renderAbilities(pokeData.abilities) +
        renderHeldItems(pokeData.heldItem);
    renderStats(pokeData.stats, pokeData.statsMinMax, pokeData.statTotal, pokeData.percent, pokeData.ev);
    document.querySelector(".stats__defense").innerHTML = renderDamageMultipliers(pokeData.typeDamage);
    document.querySelector(".moves").innerHTML = renderMoves(pokeData.moves);
    document.querySelector(".popup__btn-like").setAttribute("data-value", pokeData.primary.uId);
...
const renderTypes = (types) => {
    let tempTypeBtn = "";
    const typelphabetIds = { 1: "13", 2: "6", 3: "8", 4: "14", 5: "11", 6: "16", 7: "1", 8: "9", 9: "17", 10: "7", 11: "18", 12: "10", 13: "4", 14: "15", 15: "12", 16: "3", 17: "2", 18: "5" }
    types.forEach(e => {
...
const renderPrimaryRight = (primaryData, flavorText, ev, eggData, gender, catchRate) => {
    const color = ["Black", "Blue", "Brown", "Gray", "Green", "Pink", "Purple", "Red", "White", "Yellow"];
    let height = "";
    const heightInInches = primaryData.height * 3.2808;
    height += Math.trunc(heightInInches) + "'" + ((heightInInches - Math.floor(heightInInches)) * 12).toFixed(0) + '"  '
    ...
    return `
       ...
       <p class="primaryInfo__right-element-title">Level Calculator: </p>
                ...
                <input type="number" min="0" max="100" id="txtExp" class="input__field primaryInfo__right-text-input" placeholder="Desired Level">
                    <button data-value="${primaryData.growth.growthRate}" class="btn btn--small primary__btn">=</button>
                </div>
        ...
        <div class="primaryInfo__right-element">
            ...
            <p class="primaryInfo__right-element-data">${primaryData.baseExp} exp</p>
        </div>
        <div class="primaryInfo__right-element">
            <p class="primaryInfo__right-element-title">Base Friendship:</p>
            <p class="primaryInfo__right-element-data">${primaryData.baseFriendship}</p>
        </div>
        <div class="primaryInfo__right-element">
        ...
        <div class="primaryInfo__right-element">
            ...
            <p class="primaryInfo__right-element-data">
                <span class="primaryInfo__right-male">${gender.m}% male</span><span class="primaryInfo__right-female">${gender.f}% female</span>
            </p>
        </div>
        <div class="primaryInfo__right-element">
            <p class="primaryInfo__right-element-title">Egg Cycles:</p>
            <p class="primaryInfo__right-element-data">${eggData.hatchCount}
                <span>(${formatNumber((eggData.hatchCount + 1) * 255)}–${formatNumber((eggData.hatchCount + 1) * 257)} steps)</span>
            </p>
        </div>
        <div class="primaryInfo__right-element">
        ...
        <div class="primaryInfo__right-element">
            ...
            <p class="primaryInfo__right-element-data">
                <span class="primaryInfo__color primaryInfo__color--${primaryData.color}">
                    ${color[primaryData.color - 1]}
                </span>
            </p>
        </div>
        <div class="primaryInfo__right-element">
            <p class="primaryInfo__right-element-title">Body Type: ${upCaseFirstChr(primaryData.shape[1])}</p>
...
<h3 class="heading-3">
    ...
    document.querySelector(".stats__base").insertAdjacentHTML(htmlIncept.afterBegin,
        htmlMarkup);
    renderStatsRadar(stats);
    htmlMarkup = renderStatsTable(stats, minMax, total, percent);
    document.querySelector(".stats__base").insertAdjacentHTML(htmlIncept.beforeEnd, htmlMarkup);
}
    ...
    utillityObj.forEach(stat => {
        ...
        <tr class="table__row stats__base-${stat.key}" data-value="${stat.key}">
            ...
            <td class="table__row-bar">
                <div class="table__row-bar-data stats__base-data" width="${percent[stat.key]}%"></div>
            </td>
...
const renderStatsRadar = (stats) => {
    const chart = document.getElementById("elemBaseStats-0");
    const statsTitles = ["attack", "defense", "hp", "spAtk", "spDef", "speed"];
    const pointTitles = ["Attack", "Defense", "HP", "Special Attack", "Special Defense", "Speed"];

    const statValues = [];
    let largest = 0;
    const labelCallback = (tooltipItem, data) => {
        let labelMarkup = data.datasets[tooltipItem.datasetIndex].data[tooltipItem.index];
        return " : " + labelMarkup;
    }
...
const renderDamageMultipliers = (typeDamage) => {
    ...
    let damageKeys = [Object.keys(typeDamage[damageChategories[0]]),
    Object.keys(typeDamage[damageChategories[1]])];
    htmlMarkup += `
    <h2 class="heading-2">Damage Multiplier</h2>
    <h3 class="heading-3">`;
    damageKeys.forEach((e, i) => {
        htmlMarkup += `<span id="lblEffectiveness-${i}" class="effectiveness__tab ${i == 0 ? "effectiveness__tab--active" : ""}">${tableTiles[i]}</span>${i == 0 ? "/" : ""}`
    });
    htmlMarkup += `</h3>`
    damageKeys.forEach((e, i) => {
        htmlMarkup += `
            <table id="tblEffectiveness-${i}" class="table effectiveness ${i == 0 ? "effectiveness--visible" : ""}">`;
        e.forEach(e1 => {
            htmlMarkup += renderEffectivenessMarkup(e1, typeDamage[damageChategories[i]][e1]);
        });
        htmlMarkup +=
            `</table>`;
    });
...
const renderEVs = (ev) => {
    // const evTitles = { "hp": "HP", "attack": "Attack", "defense": "Defense", "spAtk": "Special Attack", "spDef": "Special Defense", "speed": "Speed" }
    const evTitles = { "hp": "HP", "attack": "Atk", "defense": "Def", "spAtk": "Sp.Atk", "spDef": "Sp.Def", "speed": "Speed" }
    let evTotal = 0;
...
const getChildMarkUp = (evolutions) => {
    ...
    evolutions.forEach((e, i) => {
        const evoPointRotation = (evoLength == 1) ||
            (i == 2 && evoLength == 3) ? "" : ` evolution__pointer-${i + 1}Of${evoLength}`;
...
const renderEvolutionNode = (evoNode, pointerRotation) => {
    ...
    let sectionMarkup = `${evoNode.isRoot ? `<div class="evolution__container-row">` : ``}
    <div class="evolution__pokemon">
    <img class="evolution__pokemon-image" src="../img/pokemonImages/${genName(evoNode.id, evoNode.id)}.png" alt="pokemon">
        <h4 class="heading-4 evolution__pokemon-naming">
            <span class="evolution__pokemon-name">${upCaseFirstChr(evoNode.name)}</span>
    ...
    evoNode.isRoot ? htmlMarkup += `${sectionMarkup}` :
        htmlMarkup += `
...
const renderMoveTable = (moveTable, typeTable, id) => {
    ...
    <table id="moves__${id}"  class="moves__moves table table__sortable">
        <thead>
            <tr>
                ${isLevelup ? `<th data-value="0-int" id="moves__${id}--Lvl" class="table__heading table__sortable">Lvl</th>` : ""}
                <th  ${isLevelup ? 'data-value="1-str"' : 'data-value="0-str"'} id="moves__${id}--Move" class="table__heading table__sortable">Move</th>
                <th  ${isLevelup ? 'data-value="2-data"' : 'data-value="1-data"'} id="moves__${id}--Type" class="table__heading table__sortable">Type</th>
                <th  ${isLevelup ? 'data-value="3-str"' : 'data-value="2-str"'} id="moves__${id}--Category" class="table__heading table__sortable">Category</th>
                <th  ${isLevelup ? 'data-value="4-int"' : 'data-value="3-int"'} id="moves__${id}--Power" class="table__heading table__sortable">Pwr</th>
                <th  ${isLevelup ? 'data-value="5-int"' : 'data-value="4-int"'} id="moves__${id}--Accuracy" class="table__heading table__sortable">Acc</th>
                <th  ${isLevelup ? 'data-value="6-int"' : 'data-value="5-int"'} id="moves__${id}--PP" class="table__heading table__sortable">PP</th>
            </tr>
        </thead>
        <tbody>
            ${rowMarkup}
